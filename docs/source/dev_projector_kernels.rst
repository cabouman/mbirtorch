...
**Parallel beam is the degenerate case.**  The parallel kernels are the cone kernels
with the vertical fan deleted: detector row *r* is recon slice *r*, so the row axis
rides through both kernels unchanged.  With the vertical fan goes the cone kernels'
...
2. **The per-kernel, per-device value self-check.**  On first use, each of the four
   kernels (cone back, cone forward, parallel back, parallel forward) is run against
   its torch body on a tiny problem *on the actual device that will run it*, and
   falls back if the results differ by more than the 1e-4 contract.  The answer is
   cached per device for the process.
