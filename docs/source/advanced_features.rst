...
- **Change Reconstruction Size and Shape:**
  ...
  MBIRTorch will automatically set the reconstruction array size and voxel pitch to reasonable values.
...
- **Reconstruct Objects Larger than the Field of View:**
  ...
  or end-slice "flash", along with an interior bias and slowed convergence.  MBIRTorch handles the
  ...
  - *Lateral (rows and columns):* MBIRTorch detects the case when the object appears to go
...
- **Calibrate and Control Model:**
  ...
  MBIRTorch will allow you to compensate for non-ideal data with the following parameters:
