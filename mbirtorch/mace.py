...
provided, and any callable of the same form serves as one.
:func:`temporal_filter_matrix` builds the frame-axis filter that the
hyperplane agent can apply.  :func:`mace` runs the loop in one call.
"""
...
class HyperplaneAgent:
    ...
    Args:
        ...
        filter_matrix (torch.Tensor, optional): a frame-axis filter matrix
            from :func:`temporal_filter_matrix`, applied to each batch before
            denoising.
    ...
    def _run_slab(self, w, region, device):
        ...
        if self.filter_matrix is not None:
            stack = apply_temporal_filter(stack, self.filter_matrix, axis=1)
