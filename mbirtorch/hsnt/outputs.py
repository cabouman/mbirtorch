...
from .io import _create_hyperspectral, export_hsnt_data_hdf5

log = logging.getLogger(__name__)


def fit_quality(T, W, H, dose=None, device="cpu", chunk=65536):
    """Reduced chi-square of the fit against Poisson noise, the mean of dose (T - e^-X)^2 / e^-X over the data.

    Near 1 the residual is at the noise level; well above 1 the rank is too small or the model is wrong; well below 1
    the fit follows the noise.
    ...
    Args:
        ...
        chunk (int, optional): Pixels per block. Defaults to 65536.
    ...
    for i in range(0, T.shape[0], chunk):
        ...
        res += (d * d).sum().item()
        tot += (Tb * Tb).sum().item()
        num += ((d * d) / Th.clamp_min(1e-12)).sum().item()
        den += Tb.numel()
    out = dict(relative_residual=float(np.sqrt(res / tot)))
    if dose is not None:
        out["reduced_chi2"] = dose * num / den
    return out
...
def _provenance(path, bin_indices, attrs):
    """Add the source bin index of each column and the run's attributes to an HDF5 file."""
...
def write_dehydrated(path, W4, H, out_type, bin_indices, attrs):
    ...
    Returns:
        str: the path.
    """
    export_hsnt_data_hdf5(path, [W4, H, out_type], {"dataset_type": out_type,
                                                    "dataset_modality": "hyperspectral neutron"})
    _provenance(path, bin_indices, attrs)
    ...
    with h5py.File(path, "a") as f:
        ...
        d.attrs["description"] = (f"attenuation of the average material pixel ({n_mat} pixels), sum_k mean(W_pk) H_k; "
                                  "independent of the split among components")
...
def write_denoised(path, spatial_shape, W, H, out_type, bin_indices, attrs, block_pixels=16384):
    ...
    with h5py.File(path, "w") as f:
        ...
        for v in range(V):
            for r0 in range(0, rows, block_rows):
                ...
                d[v, r0:r1] = (np.exp(-X) if out_type == "transmission" else X).astype(np.float32).reshape(r1 - r0,
                                                                                                           cols, K)
