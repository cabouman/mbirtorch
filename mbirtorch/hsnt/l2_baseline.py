"""Mothballed L2 baseline: the scikit-learn NMF dehydration of Chowdhury et al. (2025).

Kept only as a baseline for comparison plots. The package's ``dehydrate``, ``hyper_denoise`` and the command line
...
def l2_hyper_denoise(data, dataset_type='attenuation', num_materials=None, safety_factor=2, beta_loss='frobenius',
                  ...
                  verbose=1):
    """
    L2 BASELINE. Denoise a hyperspectral dataset by scikit-learn NMF dehydration and rehydration as described in:
    ...
    Example:
        >>> denoised_data = l2_hyper_denoise(data, num_materials=5, safety_factor=2)
...
def l2_dehydrate(data, dataset_type='attenuation', num_materials=None, safety_factor=2, beta_loss='frobenius',
              max_iter=300, tolerance=1e-10, batch_size=2 ** 27, subspace_basis=None, random_state=None,
              verbose=1):
    """
    L2 BASELINE. Dehydrate/compress a hyperspectral dataset by scikit-learn NMF (Frobenius or KL) as described in:
    ...
    Example:
        >>> [subspace_data, subspace_basis, dataset_type] = l2_dehydrate(data, num_materials=5, safety_factor=2)
