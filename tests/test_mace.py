...
threaded path is checked with two workers on the CPU, the checkpoint by a
round trip, and the frame-axis filter matrix against the transform it
replaces.  The agents are checked with stubs that record what they were
...
from mbirtorch.mace import (MACE, ForwardProxAgent, HyperplaneAgent, QGGMRFDenoiserAgent, Task,
                            apply_temporal_filter, mace, resolve_device_pool, temporal_filter_matrix)
