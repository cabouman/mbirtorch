...
count.  And the selection contract must hold: under the interim the forward
kernel is NOT bound on a sharded layout.

This file is also the acceptance bar for the repaired forward kernel.
Restoration means adding the kernel-forward arm here and watching it join the
torch arms at the floor; the interim rule retires when it does.
...
def test_the_back_kernel_matches_the_torch_bodies_under_sharding(geometry,
    ...
    geometries, which is why the interim withholds only the FORWARD kernel.
    """
    sinogram, weights = problem[geometry]
    ...
    # The shipped selection at two devices: torch forward, kernel back.
...
@requires_two_cuda
@pytest.mark.parametrize("geometry", ["parallel", "cone"])
def test_the_interim_withholds_the_forward_kernel_from_a_sharded_layout(
        geometry, problem):
    """The selection contract, asserted on real hardware.

    The CPU tests pin the RULE by forcing the availability gates.  This pins
    what a real multi-GPU machine actually binds, which is the thing a user
    gets.  When the forward kernel is repaired, this test is the one that
    changes.
    """
    torch_forward = (_parallel_forward_view_batch if geometry == "parallel"
                     else _cone_forward_view_batch)
    model = _build(geometry)
    ...
    assert sharded_forward is torch_forward, (
        "the forward kernel must not bind on a sharded layout while the "
        "interim stands")
    # The back body is whatever the availability gates chose, and it must be
    # the SAME choice at both device counts.
    assert sharded_back is single_back
    if "triton" in single_forward.__name__:
        # Where the kernel is usable at all, one device still gets it.
        assert single_forward is not torch_forward
